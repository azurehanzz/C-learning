...
void initi_Con(Con* addr)
{
	...
	int i = 0;
	char buffer[200];
	if (fgets(buffer, sizeof(buffer), pfread) == NULL)
	{
		perror("fgets");
		return;
	}
	while (fscanf(pfread, "%s %s %d %s %s", addr->data[i].name,
		addr->data[i].sex,
		&addr->data[i].age,
		addr->data[i].phone_num,
		addr->data[i].addr)!=EOF)
	{
		//判断是否需要扩容
		addr->sz++;
		Add_Capacity(addr);
...
void Save_data(Con* addr)
{
	...
	fprintf(pfwrite, "%-20s\t%-10s\t%-s\t%-20s\t%-30s\n", "姓名", "性别", "年龄", "手机号", "地址");
	int i;
	for (i = 0; i < addr->sz; i++)
	{
		fprintf(pfwrite, "%-20s\t%-10s\t%-d\t%-20s\t%-30s\n", addr->data[i].name,
			addr->data[i].sex,
			addr->data[i].age,
			addr->data[i].phone_num,
			addr->data[i].addr);
